...
export default function Reviews() {
  ...
  useEffect(() => {
    setLoading(true)
    api.getReviews()
      .then(r => { setReviews(r); setError(null) })
  ...
  const submitReply = async (id: number) => {
    ...
    setSubmitting(true)
    try {
      const updated = await api.replyToReview(id, draftReply.trim())
    ...
    <div>
      ...
      {!loading && reviews.length === 0 && !error && (
        <div style={{ textAlign: 'center', padding: '64px 0', color: 'var(--text2)' }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 600 }}>Отзывов пока нет</div>
          <div style={{ fontSize: 13, marginTop: 6 }}>Они появятся после первых завершённых консультаций</div>
        </div>
      )}
              ...
              <div key={r.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {r.pet_name && <div style={{ fontSize: 12, color: 'var(--text2)' }}>{r.pet_name}</div>}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2 }}>
                    <Stars value={r.rating} />
                    <span style={{ fontSize: 11, color: 'var(--text2)' }}>{fmtDate(r.created_at)}</span>
                  </div>
                </div>

                {r.text && <p style={{ fontSize: 13, color: 'var(--text)', lineHeight: 1.55, margin: 0 }}>{r.text}</p>}
                ...
                {replying === r.id ? (
                  <div style={{ marginTop: 12 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 8 }}>
                      <button
                        onClick={() => { setReplying(null); setDraftReply('') }}
                        disabled={submitting}
                        style={btnGhost}
                      >
                        Отмена
                      ...
                      <button
                        ...
                        disabled={!draftReply.trim() || submitting}
                        style={btnCoral}
                      >
